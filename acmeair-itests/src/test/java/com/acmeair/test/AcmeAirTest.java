...
//		String httpPort = "9081";
	...
	public static void init() throws Exception {
		...
		try {
			...
			System.out.println("statusTesxt:" + method.getStatusText());
			assertEquals("HTTP GET failed", HttpStatus.SC_OK, statusCode);

			String response = method.getResponseBodyAsString();
			assertTrue("Unexpected response body",
					response.contains("Sample data loaded."));
		} finally {
	...
	public void testQueryFlights() throws Exception {
		...
		try {
			...
			assertTrue(node.get("tripFlights").get(0).get("flightsOptions")
					.size() > 0);
			assertTrue(node.get("tripFlights").get(1).get("flightsOptions")
					.size() > 0);
		} finally {
			logout(sessionCookie);
		}
	...
	private void logout(Cookie sessionCookie) throws Exception {
		...
		try {
			...
			assertTrue("Unexpected response body",
					response.contains("logged out"));
		} finally {
			getMethod.releaseConnection();
		}
	}

	private Customer getCustomer(String id, Cookie sessionCookie)
			throws Exception {
		GetMethod getMethod = new GetMethod(baseURL
				+ "/rest/api/customer/byid/" + id);
		getMethod.setRequestHeader("Cookie", sessionCookie.toExternalForm());
	...
	private Customer updateCustomer(String id, Cookie sessionCookie,
			Customer customer) throws Exception {
		PostMethod method = new PostMethod(baseURL + "/rest/api/customer/byid/"
				+ id);
	...
	private JsonNode queryFlights(Cookie sessionCookie, String fromAirport,
			String toAirport) throws Exception {
